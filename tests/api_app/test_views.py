...
class JobViewSetTests(CustomViewSetTestCase):
    ...
    def test_list_query_counts(self):
        ...
        # Since the N+1 bug caused 100+ queries for 10 jobs, asserting < 35 is a safe threshold
        # that accounts for constant-time overhead queries (auth, perms, etc.)
        with CaptureQueriesContext(connection) as queries:
            response = self.client.get(self.jobs_list_uri)
            self.assertEqual(200, response.status_code)
        
        self.assertLess(len(queries), 35, msg=f"Queries matched or exceeded 35! Expected <35 to avoid N+1. Actual: {len(queries)}")
